...
/**
 * Property card for the "Aynan siz uchun mos uylar" / "O'xshash e'lonlar"
 * carousels. The whole card links to the listing detail page.
 *
...
const to = computed(() => localePath(`/listings/${props.item.id}`))
</script>

<template>
  <NuxtLink
    :to="to"
    class="group relative block h-[372px] overflow-hidden rounded-[22px] bg-surface shadow-card transition-shadow duration-300 hover:shadow-xl">
    ...
    <div
      ...
      <div
        ...
        <span
          ...
          <span class="flex items-center gap-1"
            ><IconsChartBars class="text-[13px]" />{{ item.floor }}</span
          >
        </span>
      </div>
      ...
      <div class="flex flex-1 flex-col px-1.5 pt-3">
        ...
        <p
          class="mt-1 flex items-center gap-1 text-xs text-neutral-500 line-clamp-1">
          <IconsMapPin class="shrink-0 text-sm" />{{ item.address }}
        </p>
        <div class="mt-auto flex items-center justify-between pt-3">
          ...
          <span class="flex items-center gap-1 text-xs text-neutral-400"
            >{{ item.views }}<IconsEye class="text-sm"
          /></span>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      <p
        class="mt-1 flex items-center gap-1 text-xs text-white/70 line-clamp-1">
        <IconsMapPin class="shrink-0 text-sm" />{{ item.address }}
      </p>
